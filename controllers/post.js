...
const createPost = async (req, res) => {
	...
	if (image) {
		...
		image = { src: secure_url, publicID: publicIdToBeSent };
	}
	...
	const post = await Post.create({
		...
		image,
		createdBy: userId,
		userDetails: { name: user.userName, image: user.profileImage ,phone: user.phone },
		user:user
	});
...
const getPosts = async (req, res) => {
	const { id, post_content = "", page = "1", userId = "" } = req.query;
	if (id) {
		...
		const limitCount = post_content ? Infinity : 10;
		const skipCount = (+page - 1) * limitCount;
		const _query = {};
		if (post_content) _query.caption = new RegExp(post_content, "i");
